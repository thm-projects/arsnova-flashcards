	...
	getDeadline (cardset, user_id) {
		if (!Meteor.isServer) {
			...
			if (deadline.getTime() > cardset.learningEnd.getTime() && Bonus.isInBonus(cardset._id, user_id)) {
				return (TAPi18n.__('deadlinePrologue', null, Meteor.settings.mail.language) + moment(cardset.learningEnd).locale("de").format("dddd") + TAPi18n.__('deadlineEpilogue1', null, Meteor.settings.mail.language));
			} else {
				return (TAPi18n.__('mailNotification.textDate1', null, Meteor.settings.mail.language) + moment(deadline).locale("de").format("dddd") + TAPi18n.__('mailNotification.textDate2', null, Meteor.settings.mail.language));
			}
		}
	}
