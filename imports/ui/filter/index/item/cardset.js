...
Template.filterIndexItemCardset.events({
	...
	'click .collapseCardsetInfoButton': function (event) {
		if ($(event.target).hasClass('glyphicon-collapse-up')) {
			...
			$($(event.target).data('id')).hide('slide', {direction: "up"}, 'fast');
		} else {
			...
			$($(event.target).data('id')).show('slide', {direction: "up"}, 'fast', function () {
				$('body, html').animate({scrollTop: ($(event.target).offset().top - ($('.resultItemHeaderWrapper').height() + $('.thm-header').height() - 25))}, 'fast');
			});
		}
	},
