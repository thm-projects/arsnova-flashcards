...
if (Meteor.isServer) {
  Meteor.methods({
    ...
    updateWozniak(cardset_id, card_id, grade) {
      check(cardset_id, String);
    ...
    /** Updates the learner count of the cardset
		 ...
		 * */
    updateLearnerCount(cardset_id) {
      check(cardset_id, String);
      // Make sure the user is logged in
      if (!Meteor.isServer) {
        throw new Meteor.Error('not-authorized');
      } else {
        const learnersTotal = Workload.find({ cardset_id }).count();
        const learnersBonus = Workload.find({ cardset_id, 'leitner.bonus': true }).count();
    ...
    /** Removes an user from an active bonus
		 ...
		 * */
    removeUserFromBonus(cardset_id, user_id) {
      check(cardset_id, String);
      check(user_id, String);
      const cardset = Cardsets.findOne({ _id: cardset_id }, { fields: { _id: 1, owner: 1 } });
    ...
    skipLeitnerCard(card_id, cardset_id) {
      check(card_id, String);
      check(cardset_id, String);
      if (Meteor.user()) {
