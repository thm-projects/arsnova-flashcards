...
Meteor.methods({
  addTranscriptBonus(card_id, cardset_id, user_id, date_id) {
    if (Meteor.isServer) {
      ...
      if (cardset !== undefined) {
        TranscriptBonus.upsert({ card_id }, {
          $set: {
  ...
  updateTranscriptBonusStats(cardset_id) {
    if (Meteor.isServer) {
      check(cardset_id, String);
      const cardset = Cardsets.findOne({ _id: cardset_id });
      if (cardset !== undefined && cardset.transcriptBonus !== undefined) {
        const bonusTranscripts = TranscriptBonus.find({ cardset_id: cardset._id }).fetch();
        const submissions = TranscriptBonus.find({ cardset_id: cardset._id }).count();
        const userFilter = [];
        for (let i = 0; i < bonusTranscripts.length; i++) {
          userFilter.push(bonusTranscripts[i].user_id);
        }
        const participants = Meteor.users.find({ _id: { $in: userFilter } }).count();
  ...
  getTranscriptCSVExport(cardset_id, header) {
    check(cardset_id, String);
    check(header, [String]);
    ...
    if (UserPermissions.gotBackendAccess() || (UserPermissions.isOwner(cardset.owner) || cardset.editors.includes(Meteor.userId()))) {
      ...
      content += newLine;
      const transcriptUsers = _.uniq(TranscriptBonus.find({ cardset_id }, {
        fields: { user_id: 1 },
      }).fetch().map(function(x) {
        return x.user_id;
      }), true);
      const users = Meteor.users.find({ _id: { $in: transcriptUsers } }, {
  ...
  rateTranscript(cardset_id, card_id, answer, ratingData) {
    check(cardset_id, String);
    check(card_id, String);
    ...
    if (UserPermissions.gotBackendAccess() || (UserPermissions.isOwner(cardset.owner) || cardset.editors.includes(Meteor.userId()))) {
      ...
      TranscriptBonus.update({ cardset_id, card_id }, {
        $set: {
          rating,
          reasons,
          stars,
