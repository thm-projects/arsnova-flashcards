...
Meteor.methods({
  updateUsersVisibility(visible, id) {
  ...
  updateUsersBirthName(birthname, id) {
  ...
  updateUserFullscreenSettings(presentationMode, demoMode, leitnerMode, wozniakMode) {
  ...
  updateUsersGivenName(givenname, id) {
  ...
  updateUsersNotification(mail, web, id) {
    check(mail, Boolean);
    check(web, Boolean);
    check(id, String);
  ...
  updateUsersProfileState(completed, id) {
  ...
  setLecturerRequest(user_id, request) {
  ...
  increaseUsersBalance(user_id, lecturer_id, amount) {
    check(user_id, String);
    check(lecturer_id, String);
    check(amount, Number);

    if (amount < 10) {
      const user_amount = Math.round((amount * 0.7) * 100) / 100;
      const lecturer_amount = Math.round((amount * 0.05) * 100) / 100;
  ...
  deleteUserProfile(targetUser = undefined) {
    ...
    const allPrivateUserCardsets = Cardsets.find({
      owner: user_id,
      kind: 'personal',
    }).fetch();
    ...
    const workload = Workload.find({ user_id }, { fields: { cardset_id: 1 } }).fetch();

    Workload.remove({
      user_id,
    });

    const nextDeletedUserID = LeitnerUtilities.getNextLeitnerDeletedUserID();
    ...
    const transcriptBonus = TranscriptBonus.find({ user_id }, { fields: { cardset_id: 1 } }).fetch();
    if (transcriptBonus !== undefined) {
      TranscriptBonus.remove({
        user_id,
      });
      const cardsetsValues = Utilities.getUniqData(transcriptBonus, 'cardset_id');
  ...
  /** Function saves the given language to the given user
	 ...
	 * */
  updateLanguage(selectedLanguage, id) {
    ...
    if (Meteor.isServer) {
      ...
      },
      {
        $set: {
          ...
          'count.bonusTranscripts': TranscriptBonus.find({ user_id }).count(),
        },
      });
    }
  },
  updateWorkloadCount(user_id) {
    check(user_id, String);
    if (Meteor.isServer) {
      Meteor.users.update({
        _id: user_id,
      ...
      {
        $set: {
          'count.workload': Leitner.find({ user_id }).count() + Wozniak.find({ user_id }).count(),
        },
      });
    }
  },
  /** Function saves the given colorTheme to the given user
	 ...
	 *  @param {string} id - The id of the user
	 * */
  updateColorTheme(selectedColorTheme, id) {
